...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await supabase.from('fiscalizacoes').select('*').eq('id', id).single()
...
export async function PUT(
  ...
  try {
    ...
    // Campos inválidos para o banco
    const camposInvalidos = ['id', 'created_at']
    ...
    // UPDATE sem .single() para evitar o erro de coerce
